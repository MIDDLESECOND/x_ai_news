...
所有自动 `reddit.com` 公开端点请求必须经过 `scripts/reddit_rate_limit.py` 的跨进程硬闸门；至少间隔 30 分钟、UTC 日总预算至多 2 次，失败、429 重试和手工重跑同样计数。配置只能收紧、不能放宽。正式 L1 与影子审计共享预算，不得通过 `--delay 0`、另写请求代码或并行任务绕过。影子审计每次最多轮换 1 个社区，并与正式日报错开运行；若出现成片 403/429、账号安全锁定或强制改密，停止直接访问并先复核预算与任务状态。
